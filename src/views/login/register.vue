	...
	<div :style="{height: pageH}" class="loginPage">
		<div class="loginMain">
			...
			<div class="loginBody">
				<div class="loginLeft">
					<div class="loginInfo">
						<p class="loginTitle"><span @click="back()" class="back">返回</span><span class="userLogin">用户注册</span></p>
						<div class="inputdiv">
	    				...
	    				<div class="inputdiv">
							<i class="iconfonts el-icon-lock"></i>
	    				    <input v-model="loginForm.password" @blur="removeActive" @focus="inputFocus" placeholder="请输入密码" type="password" maxlength="6"  auto-complete="new-password"/>
	    				</div>
						<div class="inputdiv">
							<i class="iconfonts el-icon-lock"></i>
						    <input v-model="quePassword" @blur="removeActive" @focus="inputFocus" placeholder="请确认密码" type="password" maxlength="6" auto-complete="new-password"/>
						</div>
						<div class="inputdiv">
						...
						<!-- <p class="rememberPassword">
							<input type="checkbox">
							同意 <span>《重联建材网平台用户使用协议》</span>
						</p> -->
	    				<input @click="handleLogin" class="loginBtn" type="button" value="注 册" />
					</div>
				</div>
...
<script>
	import { register, registerCode_get } from '@/api/index'
	...
	export default {
	  ...
	  methods: {
		  handleClick(){
			  ...
			  registerCode_get({phone:this.loginForm.phone}).then((res) => {
			  	    if (res.code == 200) {
						loading.close();
